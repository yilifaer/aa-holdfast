...
def save_webhook(request):
    """Add, edit or remove a Discord channel from a settings page.

    The URL used to live in Django admin, on the reasoning that it is a
    credential. In practice that meant every alliance needed a Django
    superuser to add a channel, which is a lot of ceremony for pasting a URL
    that Discord itself hands out from a channel's settings menu. The people
    who can already decide which alerts fire can now add the channel too.

    Returns True if the request was a webhook action, so the caller knows
    whether to keep processing the rest of its form.
    """
    ...
    if action == "add":
        name = (request.POST.get("webhook_name") or "").strip()[:100]
        url = (request.POST.get("webhook_url") or "").strip()[:500]
        if not name or not url:
            messages.error(request, "A channel needs both a name and a URL.")
        elif not url.startswith("https://discord.com/api/webhooks/"):
            # Anything else is either a typo or not a webhook, and both fail
            # later as a silent non-delivery rather than as an error.
            messages.error(
                request,
                "That does not look like a Discord webhook URL. Discord gives "
                "you one under Channel Settings > Integrations > Webhooks.",
            )
        elif Webhook.objects.filter(name=name).exists():
            messages.error(request, f"A channel called '{name}' already exists.")
        else:
            Webhook.objects.create(
                name=name,
                url=url,
                ping_type=request.POST.get("webhook_ping") or Webhook.PingType.NONE,
            )
            messages.success(request, f"Added the channel '{name}'.")
        return True

    webhook = Webhook.objects.filter(pk=request.POST.get("webhook_pk")).first()
    if not webhook:
        messages.error(request, "That channel no longer exists.")
        return True
    ...
    if action == "toggle":
        ...
        messages.success(
            request,
            f"{'Enabled' if webhook.is_enabled else 'Disabled'} '{webhook.name}'.",
        )
    elif action == "delete":
        name = webhook.name
        webhook.delete()
        messages.success(request, f"Removed the channel '{name}'.")
    return True
